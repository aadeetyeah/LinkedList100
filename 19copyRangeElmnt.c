...
void LLcopyLastnEl(struct node**,struct node**,int,int,int);
int main(){
	struct node *head=NULL;
	struct node *dest=NULL;
	int itr1,num,from,to;
	...
	display(&head);
	printf("\nEnter range of elements to be copied from src linked list: ");
	scanf("%d %d",&from,&to);
	int cnt=count(&head);
	if(from>0 && from<to && to<cnt && to>from){
		
		LLcopyLastnEl(&head,&dest,from,to,cnt);
	}else{
...
void LLcopyLastnEl(struct node **head,struct node **dest,int from,int to,int cnt){
