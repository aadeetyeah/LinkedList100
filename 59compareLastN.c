...
int listLastNCmp(struct node**,struct node**,struct node**,struct node**,int);
...
int main(){
	...
	listLastNCmp(&head1,&tail1,&head2,&tail2,3);
	//display(&head1);	
	return 0;
}

int listLastNCmp(struct node** srcHead,struct node** srcTail,struct node** destHead,struct node** destTail,int N){
	struct node *temp1=*srcHead,*temp2=*destHead;
	int l1,l2,cnt1=0,cnt;
	l1=length(srcHead);
	l2=length(destHead);
	if(l1!=l2){	
		...
		while(cnt1<l1){
			if(cnt1>=cnt){
				if(temp1->data!=temp2->data){
					printf("Linked list are not equal at %d node.\n",cnt1+1);
					exit(1);
				}
			}
			temp1=temp1->next;
