...
int concatList(struct node** srcHead,struct node**srcTail,struct node**,struct node**);
int concatNList(struct node** srcHead,struct node**srcTail,struct node**,struct node**,int);

int main(){
	int itr1,num,data;
	struct node* head1=NULL,*tail1=NULL,*head2=NULL,*tail2=NULL;
	printf("Insert number of nodes to be created for list-1.\n");
	scanf("%d",&num);
	for(itr1=0;itr1<num;itr1++){
		printf("Enter element %d: ",itr1+1);
		scanf("%d",&data);
		insertLast(&head1,&tail1,data);
	}
	printf("List-1\n");
	display(&head1);
	tail1=findTail(&head1);

	printf("Insert number of nodes to be created for list-2.\n");
	scanf("%d",&num);
	for(itr1=0;itr1<num;itr1++){
		printf("Enter element %d: ",itr1+1);
		scanf("%d",&data);
		insertLast(&head2,&tail2,data);
	}
	printf("List-2\n");
	display(&head2);
	tail2=findTail(&head2);
	printf("Enter no. of src nodes to be concatenated.\n");
	scanf("%d",&num);
	concatNList(&head1,&tail1,&head2,&tail2,num);
	display(&head2);
	return 0;
}

int concatNList(struct node** srcHead,struct node**srcTail,struct node** destHead,struct node** destTail,int N){
