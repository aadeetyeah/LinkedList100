...
int searchSecondLastOcc(struct node**,struct node**,int no);
...
int main(){
	...
	scanf("%d",&num);
	int pos=searchSecondLastOcc(&head,&tail,num);
	printf("Output: %d\n",pos);
	return 0;
}

int searchSecondLastOcc(struct node** Head,struct node** Tail,int no){
	struct node *temp=*Head;
	int cnt=1,pos1=0,pos2=0;
	if(*Head==NULL){
